...
EOM
    ...
    context 'and no config file' do
      ...
      it 'should complete with success when cwd is below cookbooks and paths are not specified' do
        result = shell_out("#{chef_client} -o 'x::default' --config-file-jail \"#{path_to('')}\"", :cwd => path_to('cookbooks/x'))
        result.error!
      end

      it 'should fail when cwd is high above and paths are not specified' do
        result = shell_out("#{chef_client} -o 'x::default' --config-file-jail \"#{path_to('')}\"", :cwd => File.expand_path('..', path_to('')))
    ...
    context 'and a config file under .chef/knife.rb' do
      ...
      it 'should load .chef/knife.rb when -z is specified' do
        result = shell_out("#{chef_client} -o 'x::default' -z --config-file-jail \"#{path_to('')}\"", :cwd => path_to(''))
